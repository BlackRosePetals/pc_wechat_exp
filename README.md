...
> 版本：2.8.20260920 | 更新日期：2026-09-20 | 适用 WeChat 4.x（已测试 4.1.9 / 4.1.10 / 4.1.12.55 / 4.1.15.11）| Windows 10/11
...
3. [Web 界面功能详解](#3-web-界面功能详解)
   ...
   - [3.9 Pull 数据源 — 让 ChatLab 直接拉取](#39-pull-数据源--让-chatlab-直接拉取)
   - [3.10 工具 — 垃圾清理、密钥提取与解密数据库](#310-工具--垃圾清理密钥提取与解密数据库)
   - [3.11 手动输入密钥（已有密钥的用户）](#311-手动输入密钥已有密钥的用户)
   - [3.12 设置 — 语音转写引擎（本地模型 / 百度语音识别）](#312-设置--语音转写引擎本地模型--百度语音识别)
...
| **导出** | 左下方 | 「聊天导出」「Pull 数据源」「员工报表」「综合报告」 |
...
![Pull 数据源页面](screenshots/18-pull-page.png)
*图 18：Pull 数据源页面 — 选择监听地址、填写端口与 Token 后一键启动，无需命令行*

![Pull 数据源运行中](screenshots/19-pull-running.png)
...
![手动输入密钥页面](screenshots/20-manual-keys.png)
...
**校验机制**：每条密钥都会对目标数据库的第 1 页做 SQLCipher 4 的 HMAC 实测——**只有真正能解开的密钥才会被保存**，写错的密钥不会被记录（也不会污染已有配置）。保存采用合并写入，不会覆盖其它数据库的密钥。
...
### 3.12 设置 — 语音转写引擎（本地模型 / 百度语音识别）

点击仪表盘「工具」模块的「**⚙️ 设置**」（地址 `/settings`），可以配置语音转文字用哪个引擎。

![设置页面截图](screenshots/21-settings-page.png)
...
- 最新版本：`wechat_exp_2.8.20260920.exe` → [点击下载](https://github.com/sunhanaix/pc_wechat_exp/releases/download/v2.8.20260920/wechat_exp_2.8.20260920.exe)

**源码地址**：[https://github.com/sunhanaix/pc_wechat_exp](https://github.com/sunhanaix/pc_wechat_exp)

> ⚠️ **发布提醒**：请先在 GitHub Releases 上传 `dist/wechat_exp_2.8.20260920.exe`（SHA-256：`BE21F61E8EBFFA06BC8A8476E4EB6B5D6FC60927F57E05A90BD8C9B44A356A5C`）并打上 `v2.8.20260920` 标签，链接方可生效。
